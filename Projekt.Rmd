...
Nulta hipoteza je da varijable nemaju linearnu ovisnost, te ako dobijemo p vrijednost manju od 0.05 odbacit
ćemo nultu hipotezu i prihvatiti alternativnu - varijable imaju linearnu ovisnost.
```{r fig.width = 14, fig.height=5}

ggplot(players_copy, aes(x =Hand , y = Position)) +
  geom_jitter() + 
  geom_smooth(method = "lm") +
  labs(title = "Ovisnost plaće o TOI")
...
Za problem određivanja veze plaća igrača i ostalih faktora koji utječu na plaća napravit ćemo kompleksniji
model koji će koristiti više regresora - model višestruke regresije.
Kao regresore uzet ćemo ?? i ??, a za reakciju plaće igrača.
...
```
```{r}
#players.lm = lm(Hand ~ Position, players_copy)
#summary(payers.lm)
```
